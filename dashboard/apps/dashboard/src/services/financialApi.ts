...
export const financialAnalyticsApi = {
  ...
  getExecutiveDashboard: (): Promise<ExecutiveDashboard> =>
    financialClient.get('/financial-analytics/executive-dashboard').then((res) => res.data),
  ...
  getFinancialKPIs: (days = 30): Promise<FinancialKPIs> =>
    financialClient.get(`/financial-analytics/kpis/${days}`).then((res) => res.data),
  ...
  getYearlyTrends: (): Promise<YearlyTrend[]> =>
    financialClient.get('/financial-analytics/yearly-trends').then((res) => res.data),

  getCurrentMonthTrends: (): Promise<CostTrend[]> =>
    financialClient.get('/financial-analytics/current-month-trends').then((res) => res.data),
...
export const foodPricesApi = {
  ...
  getFoodPrices: (params?: {
    ...
    financialClient.get('/food-prices', { params }).then((res) => res.data),
  ...
  createFoodPrice: (data: CreateFoodPrice): Promise<FoodPrice> =>
    financialClient.post('/food-prices', data).then((res) => res.data),

  // Update food price
  updateFoodPrice: (id: number, data: Partial<CreateFoodPrice>): Promise<FoodPrice> =>
    financialClient.put(`/food-prices/${id}`, data).then((res) => res.data),

  // Delete food price
  deleteFoodPrice: (id: number): Promise<{ message: string }> =>
    financialClient.delete(`/food-prices/${id}`).then((res) => res.data),

  // Get available categories
  getCategories: (): Promise<string[]> =>
    financialClient.get('/food-prices/categories').then((res) => res.data),
};
...
export const systemSettingsApi = {
  ...
  getDefaultPriceSettings: (): Promise<DefaultPriceSettings> =>
    financialClient.get('/settings/default-price').then((res) => res.data),

  // Update default price settings
  updateDefaultPriceSettings: (data: DefaultPriceSettings): Promise<DefaultPriceSettings> =>
    financialClient.put('/settings/default-price', data).then((res) => res.data),
};
...
export const wasteTargetsApi = {
  ...
  getWasteTargets: (params?: {
    ...
    financialClient.get('/waste-targets', { params }).then((res) => res.data),

  // Get targets summary
  getTargetsSummary: (): Promise<TargetSummary> =>
    financialClient.get('/waste-targets/summary').then((res) => res.data),
  ...
  getTargetPerformance: (targetId: number): Promise<{
    ...
    financialClient.get(`/waste-targets/${targetId}/performance`).then((res) => res.data),

  // Create new target
  createWasteTarget: (data: CreateWasteTarget): Promise<WasteTarget> =>
    financialClient.post('/waste-targets', data).then((res) => res.data),

  // Update target
  updateWasteTarget: (id: number, data: Partial<CreateWasteTarget>): Promise<WasteTarget> =>
    financialClient.put(`/waste-targets/${id}`, data).then((res) => res.data),

  // Delete target
  deleteWasteTarget: (id: number): Promise<{ message: string }> =>
    financialClient.delete(`/waste-targets/${id}`).then((res) => res.data),

  // Reset target progress
  resetTargetProgress: (id: number): Promise<{ message: string }> =>
    financialClient.post(`/waste-targets/${id}/reset`).then((res) => res.data),
};
...
export const restaurantPerformanceApi = {
  ...
  getPerformanceKPIs: (period_days = 30): Promise<RestaurantPerformanceKPIs> =>
    financialClient.get('/restaurant-performance/kpis', { 
      params: { period_days } 
    }).then((res) => res.data),

  // Get daily performance trends
  getDailyTrends: (period_days = 30): Promise<RestaurantPerformanceTrend[]> =>
